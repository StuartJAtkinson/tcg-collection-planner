...
| **MTGJSON** | **No** — static files only | Files only. We use the JSON shape; `.sql`/`.psql`/`.sqlite` dumps exist but are irrelevant — the app is a static page. |
...
> The choice is the same one the previous stack made: the MTGJSON stream needs every
> printing (`default_cards` drops ~9k foreign/promo entries that MTGJSON tracks).
> So the language data is **already being downloaded**. The 372 MB is already being paid for.

## Which source feeds what

The importer is **MTGJSON-primary**: it streams `AllPrintings` and cross-references
Scryfall by `identifiers.scryfallId`. The data lands in `cards.json.gz` and `sets.js`,
both generated by `gen-cards.mjs` / `gen-sets.mjs`.

| Field group | Scryfall | MTGJSON |
|---|---|---|
...
| card printings — uuid, lang, finishes, promo types | — | `AllPrintings` |
...
No API. We use the JSON. The other shapes exist but are irrelevant — there is no
database on the other end.

- **Formats**: `.json`, `.sql`, `.psql`, `.sqlite`, CSV directory, Parquet directory —
  each in bz2/gz/xz/zip. We read `.json`.
...
- **`TcgplayerSkus`** — vendor SKUs, for the basket optimiser.
...
Our importer already caches downloads for 20h (`gen-data.mjs`'s CACHE) and only
...
One caveat on caching `all_cards`: `gen-cards.mjs` loads it into memory while folding
into `cards.json.gz`. Caching the *file* makes re-runs much faster.
