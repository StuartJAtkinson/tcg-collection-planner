...
/* THE DEFAULT MUST NOT BE "THROW IT AWAY". `kindFor` read `c > 300 ? '' : ...`,
   so any group over 300 rows arrived with NO destination and applyImport's
   `if (!k) continue` dropped every card in it. The group that rule is certain to
   hit is the biggest one, which on a real collection IS the collection: measured
   on the 2,796-row export, "Main" is 1,795 rows and Apply filed 771 of 2,566
   matched cards. Reported as "the import does nothing", and it may as well have.
   Asserted on the SIZES rather than by calling kindFor, because the defect was
   never in one number - it was that some size existed for which the app's own
   suggestion was to discard your cards. No size may map to ''. */
for (const n of [1, 100, 101, 300, 301, 1795, 20000])
  assert.ok(['binder', 'box', 'deck'].includes(t.kindFor(n)),
    `a group of ${n} rows defaults to no container, so Apply would silently discard it`);
assert.strictEqual(t.kindFor(1795), 'binder', 'a 1,795-row pile is a binder, not a deck');
assert.strictEqual(t.kindFor(60), 'deck', 'a 60-row group is a deck');
