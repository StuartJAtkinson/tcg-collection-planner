// Global card search across both games — name + game filter via the standardized sidebar.
...
export default async function SearchPage({
  ...
  const slicers: FilterGroup[] = [
    { name: 'game', label: 'Game', current: game, options: games.map((g) => ({ value: g.id, label: g.name })) },
  ];
  ...
  return (
    <div>
      ...
      <div className="flex gap-6">
        <FilterSidebar
          search={{ name: 'q', value: q, placeholder: 'card name…' }}
          slicers={slicers}
          clearHref="/search"
        />
        <div className="min-w-0 flex-1">
          ...
          <div className="grid gap-3 [grid-template-columns:repeat(auto-fill,minmax(150px,1fr))]">
            ...
            {!q && <p className="text-sm text-neutral-500">Type a card name and press Apply.</p>}
          </div>
        </div>
      </div>
    </div>
