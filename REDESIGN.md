...
| 3b.2 | Functional-grouping binder builder (the "rest"): drag-orderable sort fields — colour, rarity, type, mana value, alphabetical, collector number, set — with per-group page-break option; later an "ability colour" variable (colour from mana + ability costs + rules-text signals like "red permanent"/"for each coloured permanent", soft colours), a "types" array, and a "keyword abilities" array (incl. retro cards whose reminder-less text implies a keyword, e.g. old "can't be the target of spells" = hexproof) | organising the long tail |
