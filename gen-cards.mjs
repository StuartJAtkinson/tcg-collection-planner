...
const printings = [];    // [oracle, set, number, rarity, artId, usd, treatment, finishes, lang, artist, flavour, dfc, langMask]

/* WHICH LANGUAGES A PRINTING WAS ACTUALLY PRINTED IN, folded in from
   gen-langs.mjs if it has been run. This file cannot know it: `default-cards`
   is one row per printing, and measured, 0 of 107,347 set/number pairs carry
   more than one language row. The index says 63,990 of them exist in more than
   one -- Japanese alone covers 61,628 printings where this file has 651.
   OPTIONAL ON PURPOSE. It costs a 373 MB pass to build, so a catalogue built
   without it is a valid catalogue: the mask is 0, which the app reads as "not
   known" and draws as an absent column rather than as a row of dim pips
   claiming the card was never printed in Japanese. */
const LANG_INDEX = existsSync('data/lang-index.json')
  ? JSON.parse(readFileSync('data/lang-index.json', 'utf8')) : null;
if (!LANG_INDEX) console.warn('data/lang-index.json is not here - run `node gen-langs.mjs` for the language column');
...
for await (const raw of rl) {
  ...
  printings.push([
    ...
    dfcOf(c),
    // 0 is NOT KNOWN, never "English only" - the app tells the two apart
    LANG_INDEX?.index[`${(c.set || '').toUpperCase()}/${c.collector_number}`] || 0,
  ]);
}

const json = JSON.stringify({ o: oracles, p: printings, artists, flavour, kws,
  // the vocabulary the masks are bit positions in, so the page never has to
  // keep a copy of it in step by hand
  langs: LANG_INDEX?.langs || 0 });
