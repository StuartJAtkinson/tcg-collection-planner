...
/* NO `owned` COLUMN, and it is worth saying why rather than just not writing
...
const rows = ordered.map((s) => {
  ...
  return `  [${q(esc(s.name))},${q(s.code.toUpperCase())},'${s.released_at}',${s.card_count},${
...
// a set can actually be drafted. There is no ownership here: see ownedIn().
const SETS = [
${rows.join('\n')}
];
