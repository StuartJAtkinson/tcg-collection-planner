...
export async function searchCards(p: SearchParams): Promise<SearchCard[]> {
  ...
  return (await client`
    with hold as (
      select h.card_id,
             sum(case when h.finish not in ('normal','nonfoil') then h.quantity else 0 end)::int as foil,
    ...
    grp as ( -- oracle-level rollup (falls back to the card id when there's no oracle identity)
      select coalesce(c.oracle_id, c.id) as gid,
             ...
             bool_or(coalesce(hd.foil, 0) > 0) as any_foil
      from cards c left join hold hd on hd.card_id = c.id
      group by 1
    )
    select id, name, image_small, set_code, func_total > 0 as owned,
           func_total, deck_nonfoil, deck_foil, any_foil
    from (
      select distinct on (coalesce(c.oracle_id, c.id))
             c.id, c.name, c.image_small, s.code as set_code,
      ...
      order by coalesce(c.oracle_id, c.id), (g.func_total > 0) desc, s.release_date desc nulls last
    ) reps
